    ...
    createModuleShowList(options) {
      options.forEach((item, index) => {
        this.moduleShowList.push({
          isLoaded: arrContainObj(this.openedList, index) ? true : false, //加载成功标识(是否加载过)
          sw: arrContainObj(this.openedList, index) ? true : false, //显隐开关
        });
      });
    },
