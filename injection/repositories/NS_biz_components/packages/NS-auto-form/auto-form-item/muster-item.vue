...
<script>
  ...
  import {distributor} from '@NEAP/utils/validate/index';
  ...
  export default {
    ...
    computed: {
      ...
      labelWidthConvert() {
        ...
        /*
          ...
          */
        return utils.isXType('string', this.labelWidth)
          ? this.labelWidth
          : utils.isXType('number', this.labelWidth)
      ...
      /*
         ...
         */
      isValidating() {
        const checkList = this.$store.state.AutoForm.check.checkList; //check list
        ...
        if (this.arrContainObj(this.noValidateList, this.itemType)) {
          ...
          if (this.required || this.rules) {
            return (
              utils.arrContainObj(checkList, checkKey) ||
    ...
    methods: {
      ...
      parameConver(type, parame) {
        if (utils.judgeType(parame) === 'string') {
          const map = {
            min: parseInt(parame),
            max: parseInt(parame),
      ...
      arrContainObj(arr, val) {
        return utils.arrContainObj(arr, val);
      }
    },
    model: {
      prop: 'fatherItem',
