<template>
  <!--select-->
  <div
    class="ns-area-linkage"
    :style="{ width: width + 'px', height: height + 'px' }"
    v-loading="loading"
    element-loading-text="加载中"
  >
    <el-select
      v-for="(selectPart, index) in select_info"
      :index="index"
      :key="index"
      class="ns-select area-link-part fl"
      v-if="isRendered"
      v-model="areaLinkageStroe[selectPart.modelKey]"
      :disabled="selectPart.disabled"
      :clearable="clearable"
      :placeholder="selectPart.placeholder"
      @visible-change="visibleChange(index)"
      @change="change(index)"
      :style="{
        width: selectPart.width + 'px',
        height: selectPart.height,
        'margin-right': selectPart.index === select_info.length ? 0 : areaGap + 'px',
      }"
    >
      <el-option
        v-for="item in items[selectPart.type]"
        :key="item.value"
        :label="item.label"
        :value="item.value"
...
<script>
import {strContainPlace, delsubstr} from '../../../utils/index'
import { getsSelectOption } from '../../../service/Common/auto-from';

export default {
  name: 'ns-area-linkage',
  data() {
    return {
      childAreaLinkage: '',
      areaLinkageStroe: '',
  ...
  methods: {
    ...
    initRequire(url, query) {
      if (url && query) {
        this.loading = true;
        getsSelectOption(url, { queryParam: query })
          .then(response => {
    ...
    visibleChange(index) {
      ...
      if (selectPart.requestSwitch) {
        ...
        const query =
          areaLevel === '1'
            ? { areaLevel: areaLevel }
            : { areaLevel: areaLevel, areaCode: areaCode };
    ...
    //judge the number of area-linkage components
    judgeAreaNum() {
      return this.type === 'area'
        ? 3
        : strContainPlace(this.type, 'area-', 0)
        ? parseInt(delsubstr(this.type, 'area-'))
        : 0;
    },
    getlastSelKey() {
  ...
  props: {
    ...
    dynamicQuery: { type: [Object, String, Array, Number], default: null },
    /*  ==================================================================  */
