...
class DP {
  ...
  dataSources = (vm, formitem, prop) => {
    ...
    const canCoverSW = utils.arrContainObj(this[canCoverList](), prop);
    //cover-data
  ...
  componentWidth = (vm, formitem) => {
    ...
    //not exits = > 0
    allWidth = utils.isExitsVariable(allWidth) && !isNaN(allWidth) ? allWidth : 0;
    lw = utils.isExitsVariable(lw) && !isNaN(lw) ? lw : 0;
    //judge label-width and all-width , which one is larger
    if (allWidth >= lw) {
      //no label-components' width is all-width
      return utils.arrContainObj(vm.noLabelList, this.dataSources(vm, formitem, 'item-type'))
        ? allWidth
        : allWidth - lw;
    } else {
...
let _isCoverDataSet = (vm, key, modelKey) => {
  if (utils.judgeType(vm.coverData) === 'object') {
...
let isOtherConfigDataSet = (formitem, fieldsKey, otherConfigKey) => {
  if (formitem.hasOwnProperty(otherConfigKey)) {
    ...
    if (utils.judgeType(otherConfigObj) === 'object') {
