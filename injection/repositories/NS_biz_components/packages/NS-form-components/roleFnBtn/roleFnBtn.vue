  ...
  >
    ...
    >
      ...
      <div class="moduleTit mainTit firstModuleTit">
        ...
        <span class="spanTit">{{ firstItem[firstKeyRefer['menu-name']] }}</span>
      ...
      >
        ...
        <div class="moduleTit subTit secondModuleTit">
          ...
          <span class="fl spanTit">{{ secondItem[secondKeyRefer['menu-name']] }}</span>
        ...
        >
          ...
          <div class="moduleTit subTit thirdModuleTit fl" :style="{ width: titBaseW + 'px' }">
            <span class="spanTit">{{ thirdItem[thirdKeyRefer['btnType']] }}</span>
          </div>
...
<script>
  ...
  import { getRoleFuncButton } from '../../../service/packages/NS-form-components/roleFnBtn';

  export default {
    name: 'role-funcButton',
    data() {
      return {
        testModel: [],
        ...
        openedList: [], //init opened modules
      ...
      createModuleShowList(options) {
        options.forEach((item, index) => {
          this.moduleShowList.push({
            isLoaded: utils.arrContainObj(this.openedList, index) ? true : false, //加载成功标识(是否加载过)
            sw: utils.arrContainObj(this.openedList, index) ? true : false, //显隐开关
          });
        });
      },
...
<style rel="stylesheet/scss" lang="scss">
  $base-height: 32px; //基础高度/行高
  $base-gap: 5px; //基础间距
  $base-btn-width: 88px; //按钮宽度
  ...
  .ns-roleFnBtn {
    ...
    .moduleTit {
      ...
      &.mainTit {
        ...
        span.spanTit{
          padding: 0 $base-gap 0 $base-gap * 4;
        }
        &.firstModuleTit {
          margin-top: $base-gap;
          font-weight: bold;
    ...
    .firstModule {
      ...
      .firstContent {
        ...
        .secondModuleTit {
          padding: 0 $base-gap * 2;
          margin-bottom: $base-gap;
          /* manage-range component in role-Fn-Btn component*/
          .cx-manage__range {
            padding: 0 $base-gap * 2;
          }
        }
