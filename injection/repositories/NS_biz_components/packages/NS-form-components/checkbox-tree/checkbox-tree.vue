...
<script>
  ...
  import {getInitTreeData, getChildTreeData} from '../../../service/packages/NS-form-components/checkbox-tree';
  ...
  export default {
    ...
    methods: {
       ...
       */
      getBodyData() {
        if (utils.isEmpty(this.treeData)) {
      ...
      setExpandedKeys(data, initLeval, expandedIndex) {
        ...
        if (item) {
          do {
            ...
            if (!utils.arrContainObj(arr, item[ei][ID])) {
              arr.push(item[ei][ID]);
            }
      ...
      handleCheckChange(data, checked, indeterminate) {
        ...
        if (this.multiple) {
          if (checked) {
            if (!utils.arrContainObj(this.LastCheck, ID)) {
              this.LastCheck.push(ID);
            }
            //model data 数组中不包含 新选中的 对象， 则新加入
            if (!utils.arrContainObj(this.childTreeData, model)) {
              this.childTreeData.push(model);
            }
            if (!utils.arrContainObj(this.defaultCheckedkeys, ID)) {
    ...
    watch: {
      defaultCheckedkeys(value) {
        this.$refs.checkboxTree.setCheckedKeys(this.defaultCheckedkeys);
      },
      childTreeData: {
